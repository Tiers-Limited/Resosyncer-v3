...
import { Button, message, Select, Input, Avatar, Drawer, DatePicker, Tag, Popover, Skeleton, Segmented, Badge, Modal } from 'antd';
const { TextArea } = Input;
import { PlusOutlined, SearchOutlined, UserOutlined, FolderOutlined, CloseOutlined, SmileOutlined, AppstoreOutlined, BarChartOutlined, TableOutlined, CalendarOutlined, CommentOutlined, DeleteOutlined, InboxOutlined } from '@ant-design/icons';
...
const Projects = () => {
  ...
  const handleAssigneeChange = async (projectId, employeeIds) => {
    try {
      ...
      setProjects(prev =>
        prev.map(p => (p.id === projectId ? { ...p, assignees: assignedEmployees } : p))
      );
  ...
  const handleArchiveProject = async (projectId) => {
    Modal.confirm({
      ...
      content: 'Are you sure you want to archive this project? It will be hidden from the main view but can be restored later.',
      okText: 'Archive',
      onOk: async () => {
        try {
          const { error } = await supabase
          ...
          return (
            ...
            >
              <div className="mb-4 flex items-center gap-2">
                <Badge count={columnProjects.length} style={{ backgroundColor: column.color }}>
                  <div className="text-sm font-semibold text-gray-700 pr-2">{column.label}</div>
                </Badge>
              </div>
              <div className="space-y-3 min-h-[200px]">
                {columnProjects.map(project => (
                  <div
                    ...
                    className="bg-white rounded-lg p-4 cursor-move hover:shadow-md transition-all border border-gray-200"
                  ...
                  >
                    <div className="flex items-start gap-2 mb-3">
                      {project.country_flag && (() => {
                        ...
                        if (isFlag) {
                          const flagCode = project.country_flag.replace('FLAG:', '');
                          const FlagComponent = flags[flagCode];
                          return FlagComponent ? <FlagComponent style={{ width: 24, height: 18 }} /> : null;
                        }
                        return <span style={{ fontSize: '18px' }}>{project.country_flag}</span>;
                      })()}
                      <h3 className="text-gray-900 font-medium flex-1">{project.name}</h3>
                    </div>
                    {project.project_manager && (
                      <div className="flex items-center gap-2 mb-3">
                        <Avatar
                          ...
                          style={{ flexShrink: 0 }}
                        />
                        <span className="text-sm text-gray-600">{project.project_manager.full_name}</span>
                      </div>
                    )}
                    {project.assignees && project.assignees.length > 0 && (
                      <div className="flex items-center gap-1 mt-3">
                        <CommentOutlined className="text-gray-400 text-xs" />
                        <span className="text-xs text-gray-500">{project.assignees.length}</span>
                      </div>
                    )}
                  </div>
                ))}
                <button
                  className="w-full text-left text-sm text-blue-600 hover:text-blue-700 py-2 px-2 rounded hover:bg-gray-100 transition-colors"
  ...
  const GanttView = () => {
    ...
    const statusGroups = [
      { key: 'not_started', label: 'Not started', projects: filteredProjects.filter(p => p.status === 'not_started') },
      { key: 'revision', label: 'Revision', projects: filteredProjects.filter(p => p.status === 'revision') },
      { key: 'testing', label: 'Testing', projects: filteredProjects.filter(p => p.status === 'testing') },
      { key: 'in_progress', label: 'In progress', projects: filteredProjects.filter(p => p.status === 'in_progress') },
      { key: 'planning', label: 'Planning', projects: filteredProjects.filter(p => p.status === 'planning') },
      { key: 'completed', label: 'Completed', projects: filteredProjects.filter(p => p.status === 'completed') },
      { key: 'on_hold', label: 'On Hold', projects: filteredProjects.filter(p => p.status === 'on_hold') },
    ];
        ...
        <div className="w-80 border-r border-gray-200 bg-white overflow-y-auto">
          <div className="p-4 border-b border-gray-200 sticky top-0 bg-white z-10">
            <div className="flex items-center justify-between mb-4">
              <Button size="small" onClick={() => setCurrentMonth(currentMonth.subtract(1, 'month'))}>←</Button>
              <h3 className="text-gray-900 font-semibold">{currentMonth.format('MMMM YYYY')}</h3>
              <Button size="small" onClick={() => setCurrentMonth(currentMonth.add(1, 'month'))}>→</Button>
            </div>
          ...
          {statusGroups.map(group => (
            <div key={group.key} className="border-b border-gray-200">
              <div className="px-4 py-3 bg-gray-50 flex items-center gap-2">
                ...
                <span className="text-gray-500 text-sm">{group.projects.length}</span>
              </div>
              {group.projects.map(project => (
                <div
                  key={project.id}
                  className="px-4 py-3 hover:bg-gray-50 cursor-pointer border-b border-gray-100 flex items-center gap-2"
                  onClick={() => handleRowClick(project)}
                >
                  {project.country_flag && (() => {
                    const isFlag = project.country_flag.startsWith('FLAG:');
                    if (isFlag) {
                      const flagCode = project.country_flag.replace('FLAG:', '');
                      const FlagComponent = flags[flagCode];
                      return FlagComponent ? <FlagComponent style={{ width: 20, height: 15 }} /> : null;
                    }
                    return <span style={{ fontSize: '16px' }}>{project.country_flag}</span>;
                  })()}
          ...
          <div className="border-b border-gray-200 bg-white sticky top-0 z-10">
            <div className="flex">
              {days.map((day, i) => (
                <div
                  key={i}
                  className="flex-shrink-0 text-center py-3 border-r border-gray-200 text-gray-600 text-xs"
                  style={{ width: 40 }}
                >
                  ...
                  return (
                    ...
                    >
                      <div className="absolute inset-0 flex">
                        {days.map((_, i) => (
                          <div key={i} className="flex-shrink-0 border-r border-gray-200" style={{ width: 40 }}></div>
                        ))}
                      </div>
                      {position && (
                        <div
                          ...
                          style={{
                            ...
                            width: (position.endDay - position.startDay + 1) * 40,
                          }}
                          onClick={() => handleRowClick(project)}
                        >
      ...
      <div className="border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-semibold m-0">Projects</h1>
          </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <span className="text-gray-500 ml-2">{filteredProjects.length} projects</span>
          </div>
        </div>
      </div>
          ...
          {loading ? (
            ...
            <thead className="sticky top-0 bg-gray-50 z-10">
              <tr className="border-b border-gray-200">
                <th className="text-left px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider" style={{ width: '22%' }}>
                  Project name
                </th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider" style={{ width: '11%' }}>
                  Project Manager
                </th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider" style={{ width: '11%' }}>
                  Assignees
                ...
                <th className="text-left px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider" style={{ width: '18%' }}>
                  Remarks
                </th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider" style={{ width: '9%' }}>
                  Start date
                </th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-gray-600 uppercase tracking-wider" style={{ width: '9%' }}>
                  End date
            ...
            <tbody>
              {filteredProjects.map((project, index) => (
                <tr
                  key={project.id}
                  className="border-b border-gray-100 hover:bg-gray-50 cursor-move transition-colors group"
                  onClick={() => handleRowClick(project)}
                  draggable
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                      <IconPicker
                        ...
                        onChange={(icon) => handleInlineEdit(project.id, 'country_flag', icon)}
                        onRemove={() => handleInlineEdit(project.id, 'country_flag', null)}
                      />
                      <Input
                        value={project.name}
                        onChange={(e) => {
                          e.stopPropagation();
                          handleInlineEdit(project.id, 'name', e.target.value);
                        }}
                  ...
                  <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                    <Select
                      ...
                      onChange={(value) => handleInlineEdit(project.id, 'project_manager_id', value)}
                      bordered={false}
                        ...
                        ...projectManagers.map(pm => ({
                          label: (
                            <div className="flex items-center gap-2">
                              <Avatar src={pm.user_photo} icon={<UserOutlined />} size={20} shape="circle" style={{ flexShrink: 0 }} />
                  ...
                  <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                    <Select
                      ...
                      onChange={(value) => handleAssigneeChange(project.id, value)}
                      bordered={false}
                      className="w-full hover:bg-gray-100"
                      style={{ marginLeft: -11 }}
                      suffixIcon={null}
                      ...
                      options={employees.map(emp => ({
                        label: (
                          <div className="flex items-center gap-2">
                            <Avatar src={emp.user_photo} icon={<UserOutlined />} size={20} shape="circle" style={{ flexShrink: 0 }} />
                  ...
                  <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                    <Select
                      ...
                      onChange={(value) => handleInlineEdit(project.id, 'status', value)}
                      bordered={false}
                  ...
                  <td className="px-6 py-4">
                    <Input
                      ...
                      onChange={(e) => {
                        e.stopPropagation();
                        handleInlineEdit(project.id, 'client_name', e.target.value);
                      }}
                      onClick={(e) => e.stopPropagation()}
                  ...
                  <td className="px-6 py-4">
                    <TextArea
                      ...
                      onChange={(e) => {
                        ...
                        setProjects(prev =>
                          prev.map(p => (p.id === project.id ? { ...p, remarks: newValue } : p))
                        );
                  ...
                  <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                    <DatePicker
                      value={project.start_date ? dayjs(project.start_date) : null}
                      onChange={(date) => handleInlineEdit(project.id, 'start_date', date ? date.format('YYYY-MM-DD') : null)}
                      bordered={false}
                  ...
                  <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                    <DatePicker
                      value={project.end_date ? dayjs(project.end_date) : null}
                      onChange={(date) => handleInlineEdit(project.id, 'end_date', date ? date.format('YYYY-MM-DD') : null)}
                      bordered={false}
                      className="hover:bg-gray-100 w-full"
                      style={{ marginLeft: -11 }}
                      suffixIcon={null}
...
const ProjectForm = ({ project, teams, projectManagers, employees, onClose }) => {
  ...
  return (
    <div className="space-y-6">
        ...
        >
          {projectManagers.map(pm => (
            <Select.Option key={pm.id} value={pm.id}>
              <div className="flex items-center gap-2">
                <Avatar src={pm.user_photo} icon={<UserOutlined />} size={24} shape="circle" style={{ flexShrink: 0 }} />
                <span>{pm.full_name}</span>
              </div>
            </Select.Option>
          ))}
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">Assignees (Employees)</label>
        <Select
          mode="multiple"
          value={assignees}
        ...
        >
          {employees.map(emp => (
            <Select.Option key={emp.id} value={emp.id}>
              <div className="flex items-center gap-2">
                <Avatar src={emp.user_photo} icon={<UserOutlined />} size={24} shape="circle" style={{ flexShrink: 0 }} />
                <span>{emp.full_name}</span>
              </div>
            </Select.Option>
          ))}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <DatePicker
                ...
                onChange={(date) => handleChange('start_date', date ? date.format('YYYY-MM-DD') : null)}
                className="w-full"
                size="large"
                format="MM/DD/YYYY"
              />
            ...
            <div>
              ...
              <DatePicker
                ...
                onChange={(date) => handleChange('end_date', date ? date.format('YYYY-MM-DD') : null)}
                className="w-full"
                size="large"
                format="MM/DD/YYYY"
              />
