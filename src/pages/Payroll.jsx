...
const normalizeCurrency = (currency) =>
  String(currency || "PKR")
    .trim()
    .toUpperCase() || "PKR";
const fmtMoney = (amount, currency = "PKR") =>
  `${normalizeCurrency(currency)} ${fmtCurrency(amount)}`;
    ...
    >
      ...
      >
        ...
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          ...
          >
            {fmtMoney(payroll.payable, currency)}
          </div>
          <div
            style={{
              fontSize: 9,
...
export default function EmployeeStatsPage() {
  ...
  const employeePayrollRows = employees.map((e) => {
    ...
    return {
      ...
      baseSalary,
      deduction,
      netPay: Number(Math.max(payable, 0).toFixed(2)),
      paymentDate: dayjs(`${ym}-01`).endOf("month").format("MMM DD, YYYY"),
      photo: e.user_photo || e.profile_picture_url,
    };
  });

  const currencyBreakdown = Object.values(
    employeePayrollRows.reduce((acc, row) => {
      const c = row.currency || "PKR";
      if (!acc[c])
        acc[c] = { currency: c, employees: 0, gross: 0, deduction: 0, net: 0 };
      acc[c].employees += 1;
      acc[c].gross += row.baseSalary;
      acc[c].deduction += row.deduction;
      acc[c].net += row.netPay;
      return acc;
    }, {}),
  ...
  const teamDistribution = Object.values(
    employeePayrollRows.reduce((acc, row) => {
      ...
      if (!acc[key])
        acc[key] = {
          label: key.length > 10 ? `${key.slice(0, 10)}…` : key,
          net: 0,
          deduction: 0,
        };
    ...
    .slice(0, 8);
  const maxDeptStack =
    Math.max(...teamDistribution.map((d) => d.net + d.deduction), 1) || 1;

  const trendFactors = [0.91, 1.03, 0.96, 1];
  const monthlyTrend = trendFactors.map((factor, idx) => {
    const dt = dayjs(`${ym}-01`).subtract(
      trendFactors.length - 1 - idx,
      "month",
    );
    return {
      label: dt.format("MMM"),
  ...
  const exportExcel = () => {
    ...
    const rows = employeePayrollRows.map((r) => [
      ...
      `${r.attendancePct}%`,
      r.paymentDate,
      fmtMoney(r.baseSalary - r.deduction, r.currency),
      fmtMoney(r.deduction, r.currency),
      fmtMoney(r.netPay, r.currency),
    ]);
  ...
  const exportPdf = async () => {
    ...
    const summaryLine = currencyBreakdown
      .map((item) => `${fmtMoney(item.net, item.currency)} net`)
      .join("   ");
    if (companyLogoUrl) {
      try {
        const logoData = await loadImageAsDataUrl(companyLogoUrl);
        doc.addImage(logoData, "PNG", 28, 24, 38, 38);
      } catch {
        ...
        doc.setFontSize(18);
        doc.text((companyBrand || "R").slice(0, 1).toUpperCase(), 47, 50, {
          align: "center",
        });
      }
    } else {
    ...
    const totalNet = fmtCurrency(
      employeePayrollRows.reduce((s, r) => s + r.netPay, 0),
    );
    doc.setTextColor(17, 24, 39);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    doc.text(
      `Employees: ${employees.length}   Payroll: ${fmtCurrency(payrollTotals.base)}   Net: ${totalNet}   Deduction: ${fmtCurrency(payrollTotals.deduction)}`,
      28,
      86,
    );
    if (summaryLine) {
      ...
      doc.setFontSize(10);
      doc.setTextColor(100, 116, 139);
      doc.text(`Currency breakdown: ${summaryLine}`, 28, 102, {
        maxWidth: pageWidth - 56,
      });
    }
    autoTable(doc, {
      startY: summaryLine ? 116 : 100,
      ...
      <div style={{ padding: isMobile ? "16px" : "24px 28px 36px" }}>
        {loading ? (
          ...
          >
            ...
            <div style={{ fontSize: 13, color: "var(--asp-muted)" }}>
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    {currencyBreakdown.length} currencies · {employees.length}{" "}
                    employees
                  </div>
                </div>
              </div>
              {currencyBreakdown.length === 0 ? (
                ...
                >
                  {currencyBreakdown.map((item) => {
                    const fillPct =
                      item.gross > 0
                        ? Math.round((item.net / item.gross) * 100)
                        : 0;
                    return (
                      <div
                        key={item.currency}
                      ...
                      >
                        ...
                        >
                          ...
                          <div style={{ textAlign: "right" }}>
                            ...
                            >
                              {fmtMoney(item.net, item.currency)}
                            </div>
                            <div
                              style={{
                                fontSize: 10,
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {fmtMoney(item.gross, item.currency)}
                            </div>
                          </div>
                          <div
                            style={{
                          ...
                          >
                            ...
                            >
                              {fmtMoney(item.deduction, item.currency)}
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div>
                      ...
                      <div
                        style={{
                          ...
                          alignItems: "flex-end",
                          minWidth: isMobile
                            ? Math.max(teamDistribution.length * 56, 240)
                            : "auto",
                        }}
                      >
                        {/* Axis */}
                        ...
                        <div
                          style={{
                            flex: 1,
                            display: "grid",
                            gridTemplateColumns: `repeat(${teamDistribution.length},minmax(0,1fr))`,
                            gap: 8,
                            alignItems: "end",
                            position: "relative",
                          }}
                        >
                          ...
                          {[0, 25, 50, 75, 100].map((v) => (
                            <div
                              ...
                              style={{
                                ...
                                right: 0,
                                bottom: `${(v / 100) * 132}px`,
                                borderTop: `1px dashed ${dark ? "#232328" : "#f1f5f9"}`,
                                pointerEvents: "none",
                              }}
                            />
                          ))}
                          {teamDistribution.map((d, i) => {
                            ...
                            const netH = Math.max(
                              (d.net / maxDeptStack) * 132,
                              4,
                            );
                            const dedH = Math.max(
                              (d.deduction / maxDeptStack) * 132,
                              2,
                            );
                            return (
                              <div
                                    ...
                                    <div
                                      style={{
                                        width: Math.max(
                                          ...
                                          Math.min(
                                            32,
                                            100 / teamDistribution.length,
                                          ),
                                        ),
                                        height: dedH,
                                        borderRadius: "4px 4px 0 0",
                                        background:
                                          "linear-gradient(180deg,#ef4444,#ef444488)",
                                        cursor: "pointer",
                                        transition:
                                  ...
                                  >
                                    <div
                                      style={{
                                        ...
                                        borderRadius: "4px 4px 0 0",
                                        background:
                                          "linear-gradient(180deg,#4f46e5,#7c3aed88)",
                                        cursor: "pointer",
                                        transition:
                                          "height 0.6s cubic-bezier(0.22,1,0.36,1)",
                                      }}
                      ...
                      >
                        {[
                          {
                            color: "linear-gradient(180deg,#4f46e5,#7c3aed88)",
                            label: "Net Pay",
                          },
                          {
                            color: "linear-gradient(180deg,#ef4444,#ef444488)",
                            label: "Deduction",
                          },
                        ].map((l) => (
                          <div
              ...
              >
                ...
                >
                  ...
                  {[0, 50, 100].map((v) => {
                    const val =
                      minTrendValue +
                      (maxTrendValue - minTrendValue) * (v / 100);
                    return (
                      <text
                        key={v}
                        x="44"
                  ...
                  {monthlyTrend.length > 1 &&
                    (() => {
                      const pts = monthlyTrend.map((pt, i) => {
                        const x = 70 + i * 85;
                        const range = maxTrendValue - minTrendValue || 1;
                        const y =
                          160 - ((pt.value - minTrendValue) / range) * 130;
                        return `${x},${y}`;
                      });
                      ...
                      return (
                        <polygon
                          points={`${pts.join(" ")} ${last[0]},160 ${first[0]},160`}
                          fill="url(#trendArea)"
                        />
                      );
                    })()}
                  ...
                  {monthlyTrend.map((pt, i) => {
                    ...
                    const range = maxTrendValue - minTrendValue || 1;
                    const px = 70 + (i - 1) * 85,
                      py = 160 - ((prev.value - minTrendValue) / range) * 130;
                    const cx = 70 + i * 85,
                      cy = 160 - ((pt.value - minTrendValue) / range) * 130;
                    return (
                      <line
                        key={i}
                        x1={px}
                  ...
                  {monthlyTrend.map((pt, i) => {
                    const range = maxTrendValue - minTrendValue || 1;
                    const x = 70 + i * 85,
                      y = 160 - ((pt.value - minTrendValue) / range) * 130;
                    ...
                    return (
                      <g key={pt.label}>
                        <circle
                          ...
                          cy={y}
                          r={isLast ? 7 : 5}
                          fill={isLast ? "#4f46e5" : "var(--asp-card)"}
                          stroke="#4f46e5"
                          strokeWidth="2.5"
                        />
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    {employees.length} employees · {monthLabel}
                  </div>
                </div>
                <div
                  style={{
                    ...
                    <EmpCard
                      ...
                      stats={
                        statsMap[emp.id] || {
                          ...
                          notLogged: 0,
                          dailyRecords: {},
                          effectiveDays: effectiveWDays.length,
                        }
                      }
                      wDays={wDays}
                      weekOffIndices={weekOffIndices}
                              ...
                              <div style={{ position: "relative" }}>
                                ...
                                <div
                                  style={{
                                    ...
                                    inset: -2,
                                    borderRadius: "50%",
                                    border: `1.5px solid ${rateColor(r.attendancePct)}40`,
                                    pointerEvents: "none",
                                  }}
                                />
                              </div>
                            ...
                            >
                              ...
                              >
                                <div
                                  style={{
                                    ...
                                    background: rateColor(r.attendancePct),
                                    borderRadius: 2,
                                  }}
                                />
                              </div>
                              <span
                                style={{
                                  fontSize: 12,
                                  fontWeight: 700,
                                  color: rateColor(r.attendancePct),
                                  fontFamily: "'JetBrains Mono',monospace",
                                  minWidth: 30,
                                }}
                              >
                    ...
                    <tfoot>
                      <tr style={{ background: dark ? "#18181c" : "#fafbfd" }}>
                        <td
                          ...
                          style={{
                            padding: "12px 16px",
                            borderTop: `2px solid var(--asp-border)`,
                            fontSize: 12,
                            fontWeight: 800,
                            color: "var(--asp-text)",
                          }}
                        ...
                        >
                          {fmtCurrency(payrollTotals.base)}
                        </td>
                        <td
                          style={{
                            padding: "12px 16px",
                        ...
                        >
                          {fmtCurrency(payrollTotals.deduction)}
                        </td>
                        <td
                          style={{
                            padding: "12px 16px",
