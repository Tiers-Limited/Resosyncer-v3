...
const statusOf = (row) => {
  ...
  if (row.expires_at && dayjs(row.expires_at).isBefore(dayjs())) return "expired";
  if (row.max_uses != null && row.times_used >= row.max_uses) return "exhausted";
  return "active";
};
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div style={{ fontSize: 13, color: "#94a3b8", marginTop: 2, fontWeight: 500 }}>{label}</div>
        {sub && <div style={{ fontSize: 11, color: "#cbd5e1", marginTop: 1 }}>{sub}</div>}
      </div>
    </motion.div>
  );
}
    ...
    <Tooltip title={copied ? "Copied!" : "Copy code"}>
      <button
        ...
        style={{
          ...
          color: copied ? "#10b981" : "#94a3b8", transition: "all 0.15s",
        }}
      >
...
export default function AdminPromoCodes() {
  ...
  const openCreate = () => {
    form.resetFields();
    form.setFieldsValue({ discount_type: "percent", is_active: true, code: generateCode() });
    setEditingCode(null);
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (editingCode) {
        ...
        const { error } = await supabase.from("promo_codes").insert([{ ...insertPayload, times_used: 0 }]);
        if (error) throw error;
  ...
  const handleToggle = async (record, is_active) => {
    try {
      const { error } = await supabase
        ...
        .eq("id", record.id);
      if (error) throw error;
      setCodes(codes.map((c) => c.id === record.id ? { ...c, is_active } : c));
  ...
  const expiringSoon = codes.filter((c) => {
    ...
    return dayjs(c.expires_at).diff(dayjs(), "day") <= 7 && dayjs(c.expires_at).isAfter(dayjs());
  }).length;
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 130,
      render: (_, r) => {
        const icons = { percent: <Percent size={12} />, fixed: <DollarSign size={12} />, trial: <Zap size={12} /> };
        const colors = { percent: "#6366f1", fixed: "#10b981", trial: "#f59e0b" };
    ...
    },
    {
      ...
      width: 130,
      render: (_, r) => {
        if (!r.expires_at) return <span style={{ fontSize: 12, color: "#cbd5e1" }}>Never</span>;
        const d = dayjs(r.expires_at);
        const diff = d.diff(dayjs(), "day");
        const expired = d.isBefore(dayjs());
        return (
          <div style={{ fontSize: 12 }}>
            <div style={{ fontWeight: 600, color: expired ? "#b91c1c" : diff <= 7 ? "#b45309" : "#0f172a" }}>
              {d.format("MMM D, YYYY")}
            </div>
            <div style={{ color: "#94a3b8", marginTop: 1 }}>
              {expired ? "Expired" : diff === 0 ? "Expires today" : `${diff}d left`}
            </div>
          </div>
        );
      },
    },
    {
      ...
      render: (_, r) => {
        ...
        if (!plans || plans.length === 0) return <span style={{ fontSize: 12, color: "#cbd5e1" }}>All plans</span>;
        return (
  ...
  return (
    <div style={{
      ...
      >
        <div>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "#0f172a" }}>Promo Codes</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, marginBottom: 24 }}>
        <StatCard icon={Tag}      label="Total Codes"   value={codes.length}  color="#6366f1" />
        <StatCard icon={Check}    label="Active"        value={active}         color="#10b981" />
        <StatCard icon={BarChart2} label="Total Uses"   value={totalUses}      color="#3b82f6" />
        <StatCard icon={Calendar} label="Expiring Soon" value={expiringSoon}   color="#f59e0b" sub="within 7 days" />
      </div>
      ...
      >
        ...
        {loading ? (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 80, flexDirection: "column", gap: 12 }}>
            ...
            <span style={{ color: "#94a3b8", fontSize: 14 }}>Loading codes…</span>
          </div>
        ) : (
          <Table
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{
              ...
              background: editingCode ? "#fef3c7" : "linear-gradient(135deg,#7c3aed,#7c3aed)",
              display: "flex", alignItems: "center", justifyContent: "center",
            }}>
              {editingCode ? <Pencil size={16} color="#d97706" /> : <Tag size={16} color="#fff" />}
            </div>
            <div>
              <div style={{ fontWeight: 700, fontSize: 16, color: "#0f172a" }}>
                {editingCode ? `Edit "${editingCode.code}"` : "Create Promo Code"}
              </div>
              <div style={{ fontSize: 12, color: "#94a3b8" }}>
                {editingCode ? "Update discount code configuration" : "Set up a new discount or promotional code"}
              </div>
            </div>
          </div>
        ...
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          <div style={{ padding: "24px 28px", display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr auto auto", gap: 12, alignItems: "end" }}>
                <Form.Item name="code" label={<span className="form-label">Promo Code</span>}
                  rules={[{ required: true, message: "Required" }]} style={{ margin: 0 }}>
                  <Input
                    placeholder="SUMMER25"
                    size="large"
                    style={{ borderRadius: 10, fontFamily: "monospace", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}
            ...
            {/* Description */}
            <Form.Item name="description" label={<span className="form-label">Description</span>} style={{ margin: 0 }}>
              <Input placeholder="e.g. Summer 2025 campaign, 25% off all plans" size="large" style={{ borderRadius: 10 }} />
            </Form.Item>
            ...
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <Form.Item name="discount_type" label={<span className="form-label">Type</span>}
                  rules={[{ required: true, message: "Required" }]} style={{ margin: 0 }}>
                ...
                <Form.Item name="discount_value" label={
                  <span className="form-label">
                    {discountType === "percent" ? "Percentage (%)" : discountType === "fixed" ? "Amount ($)" : "Extra Days"}
                  </span>
                } rules={[{ required: true, message: "Required" }]} style={{ margin: 0 }}>
                  <InputNumber
                    min={0}
                    ...
                    placeholder={discountType === "percent" ? "25" : discountType === "fixed" ? "10.00" : "14"}
                    addonAfter={discountType === "percent" ? "%" : discountType === "fixed" ? "$" : "days"}
                  />
                </Form.Item>
              </div>
            </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <Form.Item name="max_uses" label={
                  ...
                  <InputNumber
                    ...
                    size="large" style={{ width: "100%", borderRadius: 10 }}
                  />
                </Form.Item>
                <Form.Item name="max_uses_per_user" label={
                  <span className="form-label" style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <Form.Item name="expires_at" label={
                  <span className="form-label" style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <Form.Item name="min_plan_price" label={
                  <span className="form-label" style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <DollarSign size={13} /> Min Plan Price ($)
                  </span>
                } style={{ margin: 0 }}>
                  <InputNumber
                    min={0} precision={0} placeholder="No minimum"
                    size="large" style={{ width: "100%", borderRadius: 10 }}
                  />
                </Form.Item>
              </div>
            ...
            {/* Applicable plans */}
            <div style={{ height: 1, background: "#f1f5f9" }} />
            <Form.Item name="applicable_plans" label={
              <span className="form-label">Applicable Plans <span style={{ color: "#94a3b8", fontWeight: 400 }}>(blank = all plans)</span></span>
            } style={{ margin: 0 }}>
              <Select
                mode="multiple" size="large" placeholder="Select plans or leave blank for all"
          ...
          <div style={{
            ...
            <button type="submit" onClick={() => form.submit()} disabled={submitLoading} style={{
              display: "flex", alignItems: "center", gap: 8,
